...
Should it cover the database? - this can be a later feature

Example flask security: [Api-security-and-Best-Practices](https://github.com/Genza999/Api-security-and-Best-Practices)

## Main Objectives:

- Template frontend for ADMIN and USER management of tokens, password reset, etc…
- Template backend for API; core of the objective is this

Could look at this project as a good example: [full-stack-fastapi-template](https://github.com/tiangolo/full-stack-fastapi-template/tree/master)

## Before any code is written:

- **Dependency Management**
    - Regular audit of package dependencies — safety / Pyup / Dependent
    - CI/CD pipelines - Synk
- **Code Hygiene**
    - Linting (Black)
    - Static code analytic for common security issues (Bandit)
...
- **Access Controls**
    - RBAC
    - ABAC
    - Libraries: (definitely looks like pycasbin - through flask-auth - might be the best)
...
- **Session management**
    ...
    - Session encryption
- **Data**
    - DB encryption rest/transit [cryptography] [going by AES-256 standards]
    - Data integrity checks (checksums/hashes)
    - Automatic de-identification/pseudonymization for test/dev environments
...
- **General Security**
    - CSRF protection
        ...
        - Flask-WTF
- **DB**
    - ORM (SQLALCHEMY or PYMONGO) to help prevent SQL injection/XSS attacks
        - SQLAlchemy
- **API**
    - Rate limiting ([flask-limiter](https://github.com/alisaifee/flask-limiter))
        - Dependency is having some type of cache-DB
    - Versioning
    - Removal of hard deletion - e.g., keeping data for X years based on X state law
    - Error handling - custom error pages - prevent leakage of sensitive data; 400/500
    - RBAC / ABAC
    - Documentation
        - Swagger
- **Communication**
    - Secure headers (HTTPS)
    - Content Security Policy (CSP): Implement CSP headers to mitigate the risk of XSS attacks by specifying which dynamic resources are allowed to load.
    - SRI - subresource integrity for scripts and stylesheets;

## Infrastructure specific not covered here:
- Disaster Recovery
- Deployment - not worrying about that, saving that for the cloud provider

Perhaps future -> have a YAML build file for AWS, Azure, GCP for one quick deployment with the appropriate infrastructure setup

Idea -> Perhaps build off of FAB? Currently used by Superset and Airflow
