...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if api_token.is_token_expired():
            print('Token is expired. Generating new token for user: ' + api_token.username)
            # Update the token and commit if necessary (consider security implications)
            api_token.update_token()
            db.session.commit()
            return {'message': 'Token is expired. Please retrieve your new token from your account by going to /api-token'}, 401
