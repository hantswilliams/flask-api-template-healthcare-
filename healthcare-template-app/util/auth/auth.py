...
from models.models import (
    ...
    User,
    UserActivityLog,
)  # Import the User and UserActivityLog models
import re
...
def validate_password(password, **kwargs):
    ...
    min_password_length = kwargs.get(
        "min_password_length", current_app.config.get("PASSWORD_MIN_LENGTH", 8)
    )
    pass_req_uppercase = kwargs.get(
        "pass_req_uppercase", current_app.config.get("PASSWORD_REQ_UPPERCASE", True)
    )
    pass_req_lowercase = kwargs.get(
        "pass_req_lowercase", current_app.config.get("PASSWORD_REQ_LOWERCASE", True)
    )
    pass_req_digital = kwargs.get(
        "pass_req_digital", current_app.config.get("PASSWORD_REQ_DIGIT", True)
    )
    pass_req_special = kwargs.get(
        "pass_req_special", current_app.config.get("PASSWORD_REQ_SPECIAL", True)
    )
