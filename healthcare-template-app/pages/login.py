...
def login():
    ...
    if current_user.is_authenticated:
        return redirect(url_for('profile_pages.profile'))  # Redirect to the profile page if already logged in

    if request.method == 'GET':
        return render_template('login.html', twoFactor = twoFactor)  # Render the login page on GET requests
    ...
    if user and user.lockout_until and user.lockout_until > datetime.now():
        return jsonify({'success': False, 'message': 'Account locked. Please try again later.', 'remaining_attempts': 0})
    ...
    if user and check_password_hash(user.password, password):
        ...
        if password_age > timedelta(days=90):
            ...
            db.session.commit()
            login_user(user)
            return jsonify({'success': True, 'redirect': url_for('profile_pages.profile')})
            ...
            return jsonify({'success': False, 'message': 'Invalid 2FA token', 'remaining_attempts': remaining_attempts})
        ...
        return jsonify({'success': False, 'message': 'Invalid username or password', 'remaining_attempts': remaining_attempts})
